...
import subprocess
from cuetools import TrackData, AlbumData  # type: ignore
...
class Track(TrackData):
    ...
    @classmethod
    def from_track_data(
        cls, track: TrackData, base_dir: Path, duration: float
    ) -> Track:
        parent = track.model_dump()
        parent['file'] = (base_dir / parent['file']).resolve()

        return cls(duration=duration, **parent)
...
class Album(AlbumData):
    ...
    def from_album_data(cls, album: AlbumData, base_dir: Path) -> Album:
        ...
        for track_cue in sorted(album.tracks, reverse=True, key=lambda x: int(x.track)):
            offset, duration = _get_offset_duration(str(base_dir), track_cue, offset)
            tracks.append(Track.from_track_data(track_cue, base_dir, duration))
...
def _get_offset_duration(
    ...
    if track_cue.index00 is not None:
        offset = float(track_cue.index00.frames) / 75.0
    elif track_cue.index01 is not None:
        offset = float(track_cue.index01.frames) / 75.0
    duration = (
        next_offset - offset
